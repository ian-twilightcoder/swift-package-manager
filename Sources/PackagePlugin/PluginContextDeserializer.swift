...
internal struct PluginContextDeserializer {
    ...
    mutating func target(for id: WireInput.Target.Id, pluginGeneratedSources: [URL] = [], pluginGeneratedResources: [URL] = []) throws -> Target {
        if let target = targetsById[id], target.sourceModule?.pluginGeneratedSources.count == pluginGeneratedSources.count, target.sourceModule?.pluginGeneratedResources.count == pluginGeneratedResources.count { return target }
